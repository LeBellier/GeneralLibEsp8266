...
WiFiServer ftpSvr( FTP_CTRL_PORT);
WiFiServer dataSvr( FTP_DATA_PORT_PASV);
...
boolean FtpServer::dataConnect() {
	...
	if (!data.connected()) {
		while (!dataSvr.hasClient() && millis() - startTime < 10000) {
			//delay(100);
			yield();
		}
		if (dataSvr.hasClient()) {
			data.stop();
			data = dataSvr.available();
#ifdef FTP_DEBUG
